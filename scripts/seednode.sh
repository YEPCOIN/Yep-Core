...
git clone https://github.com/Liquid369/yep
cd yep/src
cd ..
prepare_system
create_swap
...
rm -rf *
touch yep.conf
echo "rpcuser=user" >> yep.conf
echo "rpcpassword=pass123" >> yep.conf
echo "server=1" >> yep.conf
echo "daemon=1" >> yep.conf
echo "listen=1" >> yep.conf
echo "rpcallowip=127.0.0.1" >> yep.conf
