...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zyep spend tx %s already in block %d",
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
   ...
   // Unspendable Value (coins burn) can cause a negative nMint value
    if (pindex->nMint < 0)
        pindex->nMint = 0;
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zyepSpent: %s\n",
...
/*
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert YEP to zyep
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    ...
    if (CzyepStake* zyep = dynamic_cast<CzyepStake*>(stake)) {
        ...
        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zyep stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->Iszyep() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zyep fails context checks", __func__));
        ...
        if (isBlockFromFork) {
            ...
            if(haszyepInputs){
                for (const CTxIn& zyepInput : zyepInputs) {
                    CoinSpend spend = TxInToZerocoinSpend(zyepInput);
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zyep mints and %d zyep spends\n", __func__, nMints, nSpends);
