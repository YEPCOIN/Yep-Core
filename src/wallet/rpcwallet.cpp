...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturns the current yep address for receiving payments to this account.\n"
...
UniValue burn(const UniValue& params, bool fHelp){
     if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "This command is used to Burn YEP Coins \n"
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"toyepaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nSent an amount from an account to a yep address.\n"
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const string& name_, keys) {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid yep address: ")+name_);
...
UniValue getwalletinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total YEP balance of the wallet\n"
...
UniValue listmintedzerocoins(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    set<CMintMeta> setMints = pwalletMain->zyepTracker->ListMints(true, fMatureOnly, true);
...
UniValue listzerocoinamounts(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    set<CMintMeta> setMints = pwalletMain->zyepTracker->ListMints(true, true, true);
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    ...
    bool fMintChange = params[1].get_bool();        // Mint change to zyep
    bool fMinimizeChange = params[2].get_bool();    // Minimize change
    std::string address_str = params.size() > 3 ? params[3].get_str() : "";

    vector<CZerocoinMint> vMintsSelected;

    return DozyepSpend(nAmount, fMintChange, fMinimizeChange, vMintsSelected, address_str);
}
...
UniValue spendzerocoinmints(const UniValue& params, bool fHelp)
{
    ...
    return DozyepSpend(nAmount, false, true, vMintsSelected, address_str);
}


extern UniValue DozyepSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str)
{
...
UniValue resetspentzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzyepTracker* zyepTracker = pwalletMain->zyepTracker.get();
    set<CMintMeta> setMints = zyepTracker->ListMints(false, false, false);
...
UniValue exportzerocoins(const UniValue& params, bool fHelp)
{
    ...
    CzyepTracker* zyepTracker = pwalletMain->zyepTracker.get();
    set<CMintMeta> setMints = zyepTracker->ListMints(!fIncludeSpent, false, false);
...
UniValue spendrawzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 4 || params.size() > 5)
        throw runtime_error(
            ...
            "5. \"address\"          (string, optional) yep address to spend to. If not specified, spend to change add.\n"
...
UniValue clearspendcache(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 0)
        throw runtime_error(
            "clearspendcache\n"
            "\nClear the pre-computed zyep spend cache, and database.\n" +
            HelpRequiringPassphrase() + "\n"
...

    {
        ...
        while (nTries < 100) {
            ...
            if (fLocked) {
                if (zyepTracker->ClearSpendCache()) {
