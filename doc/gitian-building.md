...
*Setup instructions for a gitian build of yep using a Debian VM or physical system.*
...
Clone the git repositories for yep and gitian and then checkout the yep version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/yep-crypto/yep.git
cd yep
...
Follow the instructions in [doc/release-process.md](release-process.md) in the yep repository
...
To build yep (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the yep repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/yep/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/yep-crypto/yep
...
For example:
```bash
URL=https://github.com/crowning-/yep.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit yep=${COMMIT} --url yep=${URL} ../yep/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit yep=${COMMIT} --url yep=${URL} ../yep/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit yep=${COMMIT} --url yep=${URL} ../yep/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/yep-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/yep-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/yep-build.assert
```
...
[yep/gitian.sigs](https://github.com/yep-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
