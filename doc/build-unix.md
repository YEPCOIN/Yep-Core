...
memory available when compiling Yep Core. With 512MB of memory or less
...
above instead of `libssl-dev`. Yep Core does not support the use of OpenSSL 1.1, though compilation is still possible
...
# Pick some path to install BDB to, here we create a directory within the yep directory
BDB_PREFIX="${yep_ROOT}/db4"
mkdir -p $BDB_PREFIX
...
# Configure Yep Core to use our own-built instance of BDB
cd $yep_ROOT
