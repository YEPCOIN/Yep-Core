...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='yep.conf'
CONFIGFOLDER='/root/.yep'
COIN_DAEMON='yepd'
COIN_CLI='yep-cli'
COIN_PATH='/usr/local/bin/'
COIN_REPO='https://github.com/Liquid369/yep'
COIN_TGZ='https://github.com/yep/yep/releases/download/4.0.0/Yep-4.0.0-x86_64-linux-gnu.tar.gz'
COIN_ZIP=$(echo $COIN_TGZ | awk -F'/' '{print $NF}')
COIN_NAME='Yep'
COIN_PORT=30361 #Updated Port
RPC_PORT=51473
...
# unzip yep.zip
#tar xvzf yep-3.1.0-x86_64-linux-gnu.tar.gz
#apt-get install -y megatools
tar -xzf Yep-4.0.0-x86_64-linux-gnu.tar.gz
cd Yep-4.0.0/bin
chmod -R 775 *
cp * $COIN_PATH
cd ..
...
function blocks() {
	wget https://github.com/yep/yep/raw/master/blocks.sh
	chmod 777 blocks.sh 
	bash blocks.sh
...
function save_key() {
	cd /root/.yep
	mv /root/.yep/yep.conf /root/.yep/yep_old.conf
	cp /root/.yep/yep_old.conf /root
}

function restore_key() {
	cd /root/.yep
	rm masternode.conf
	cp /root/yep_old.conf /root/.yep
	mv /root/.yep/yep_old.conf /root/.yep/yep.conf
}
...
function backup() {
	...
	rm -rf blocks-
	rm blocks.sh
	zip -r backupdg.zip /root/.yep
	cp /root/.yep/backupdg.zip /root
...
function prepare_system() {
 cd ~
   wget https://gist.githubusercontent.com/Liquid369/eca7f89b6c4e63f9b328d92a4f508626/raw/ba14590e3f600acdf8683eeb16cc8b2f9c8fb23e/yep.txt
#  wget https://gist.githubusercontent.com/hoserdude/9661c9cdc4b59cf5f001/raw/5972d4d838691c1a1f33fb274f97fa0b403d10bd/doge.txt
  cat yep.txt
printf "%s\n"
echo "Yep MN installer Depends Starting"
